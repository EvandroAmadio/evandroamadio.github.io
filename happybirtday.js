    ...
    if (this.phase === "firework") {
      if (!this.spawned) {
        ...
        if (this.tick >= this.reachTime) {
          ...
          let count = (opts.fireworkBaseShards + opts.fireworkAddedShards * Math.random()) | 0;
          let angle = Tau / count, cos = Math.cos(angle), sin = Math.sin(angle), x = 1, y = 0;
      ...
      if (this.circleCreating) {
        ...
        let armonic = -Math.cos(proportion * Math.PI) / 2 + 0.5;
        ctx.beginPath();
